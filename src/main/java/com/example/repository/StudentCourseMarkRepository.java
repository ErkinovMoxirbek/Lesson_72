...
public interface StudentCourseMarkRepository extends CrudRepository<StudentCourseMarkEntity,Integer >  {
    @Query("FROM StudentCourseMarkEntity where studentId = :student and createdDate = :created_date " +
            "order by mark ")
    StudentCourseMarkEntity findByStudentIdAndCreatedDateOrderByMark(StudentEntity student, LocalDateTime created_date);

    List<StudentCourseMarkEntity> findAllByStudentIdAndCreatedDateBetween(StudentEntity studentId, LocalDateTime fromDate, LocalDateTime toDate);
    ...
    Page<StudentCourseMarkEntity> findAll(Pageable pageable);

    Page<StudentCourseMarkEntity> findAllByStudentId(Integer id, Pageable pageable);

    Page<StudentCourseMarkEntity> findAllByCourseId(Integer id, Pageable pageable);
